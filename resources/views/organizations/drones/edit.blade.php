...
<form action="{{route('users.update', $user->id)}}" method="post">
    ...
    <div class="modal-header">
      <h1 class="text-center fs-5" id="staticBackdropLabel"> {{__('Updating '.$user->name)}} </h1>
    </div>
    <div class="modal-body">

      <div class="form-group">
        <label class="form-label"> {{__('Organization')}} <span class="text-red">*</span></label>
    ...
    <div class="row justify-content-between">
      <a href="{{route('users.index')}}" class="btn btn-secondary mt-2"> {{__('Back')}} </a>
