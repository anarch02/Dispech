    ...
    <div class="mt-2">
      <table class="table">
        <tbody>
          ...
          <tr>
            <td>{{__('Период на разрешения')}}</td>
            <td>
              {{$application->startDate}} <br>
              {{$application->finishDate}}
            </td>
          </tr>
      ...
      <div class="modal-content">
        <form action="{{route('admin.application.status', $application->id)}}" method="post" id="application-status-update">
          ...
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="staticBackdropLabel"> {{__('Причина отказа')}} </h1>
